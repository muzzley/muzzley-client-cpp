...
template<typename K, typename V>
muzzley::duo<K, V>::duo(K kp, V vp) :
	first(kp), second(vp), third(-1) {
}

template<typename K, typename V>
muzzley::duo<K, V>::~duo() {
	if (muzzley::is_pointer<V>::value) {
		delete this->second;
	}
}

template<typename V>
muzzley::str_map<V>::str_map(size_t hash_size) :
...
muzzley::str_map<V>::~str_map() {
	...
	for (muzzley::str_map<V>::bucket_iterator k = this->__hash_values->begin(); k != this->__hash_values->end(); k++) {
		delete *k;
	}
...
template<typename V>
void muzzley::str_map<V>::set_deleted_key(string k) {
}

template<typename V>
void muzzley::str_map<V>::clear() {
}

template<typename V>
void muzzley::str_map<V>::resize(size_t k) {
}

template<typename V>
typename muzzley::str_map<V>::iterator muzzley::str_map<V>::begin() {
	return iterator(this->__hash_values);
}

template<typename V>
typename muzzley::str_map<V>::iterator muzzley::str_map<V>::end() {
	return iterator(this->__hash_values, this->__hash_values->size());
}

template<typename V>
typename muzzley::str_map<V>::iterator muzzley::str_map<V>::begin_by_key() {
	return iterator(this->__hash_values);
}

template<typename V>
typename muzzley::str_map<V>::iterator muzzley::str_map<V>::end_by_key() {
	return iterator(this->__hash_values, this->__hash_values->size());
}

template<typename V>
typename muzzley::str_map<V>::ordered_iterator muzzley::str_map<V>::begin_by_value() {
	return ordered_iterator(this->__hash_values);
}

template<typename V>
typename muzzley::str_map<V>::ordered_iterator muzzley::str_map<V>::end_by_value() {
	return ordered_iterator(this->__hash_values, this->__hash_values->size());
}

template<typename V>
void muzzley::str_map<V>::insert(pair<string, V> v) {
	this->insert(v.first, v.second);
}

template<typename V>
void muzzley::str_map<V>::insert(pair<size_t, V> v) {
	this->insert(v.first, v.second);
}

template<typename V>
void muzzley::str_map<V>::insert(string k, V v) {
	unsigned int hash = muzzley::djb2(k);

	vector<duo<string, V>*>* bucket = this->__hash_table[hash];
	if (bucket != NULL) {
		for (muzzley::str_map<V>::bucket_iterator kt = bucket->begin(); kt != bucket->end(); kt++) {
			if ((*kt)->first == k) {
				if (muzzley::is_pointer<V>::value) {
					delete (*kt)->second;
				}
...
template<typename V>
void muzzley::str_map<V>::insert(size_t k, V v) {
	string oss;
	muzzley::tostr(oss, k);
	duo < string, V > *d = new duo<string, V>(oss, v);
	unsigned int hash = muzzley::djb2(d->first);
...
template<typename V>
V& muzzley::str_map<V>::at(size_t idx) {
	return this->__hash_values->at(idx)->second;
}

template<typename V>
typename muzzley::str_map<V>::iterator muzzley::str_map<V>::find(string key) {
	unsigned int hash = muzzley::djb2(key);
	...
	for (muzzley::str_map<V>::bucket_iterator k = bucket->begin(); k != bucket->end(); k++) {
		if ((*k)->first == key) {
...
template<typename V>
void muzzley::str_map<V>::erase(muzzley::str_map<V>::iterator i) {
	unsigned int hash = muzzley::djb2(i->first);
...
template<typename V>
size_t muzzley::str_map<V>::size() {
	return this->__hash_values->size();
}

template<typename V>
void muzzley::str_map<V>::sort() {
	std::sort(this->__hash_values->begin(), this->__hash_values->end(), muzzley::_compare<V>);
	for (size_t k = 0; k != this->__hash_values->size(); k++) {
		this->__hash_values->at(k)->third = k;
	}
...
template<typename V>
void muzzley::str_map<V>::move(string name, int offset) {
	unsigned int hash = muzzley::djb2(name);
...
template<typename V>
V& muzzley::str_map<V>::operator[](int idx) {
	return this->at((size_t) idx);
}

template<typename V>
V& muzzley::str_map<V>::operator[](const char* name) {
	V& ret =  this->find(string(name))->second;
	return ret;
}

template<typename V>
V& muzzley::str_map<V>::operator[](string& name) {
	return this->find(name)->second;
}

template<typename V>
void muzzley::str_map<V>::print(ostream& out) {
...
template<typename V>
muzzley::duo<string, V>* muzzley::str_map<V>::_at(size_t idx) {
	return this->__hash_values->at(idx);
}

template<typename V>
typename muzzley::str_map<V>::hash_iterator muzzley::str_map<V>::_find(string key) {
	unsigned int hash = muzzley::djb2(key);
...
template<typename V>
bool muzzley::_compare(duo<string, V>* lh, duo<string, V>* rh) {
	return lh->first.compare(rh->first) <= 0;
}

template<typename V>
void muzzley::str_map<V>::_rellocate(duo<string, V>* val, size_t offset) {
	unsigned int hash = muzzley::djb2(val->first);
	...
	val->third += offset;
	muzzley::tostr(val->first, val->third);
	hash = muzzley::djb2(val->first);
